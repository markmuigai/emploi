...
class Blog extends Model
{
    ...
    public function getPreviewAttribute(){
        ...
        if($max_length > 250)
            return str_replace(strip_tags(substr($this->contents, 0,160).'...'), '&rsquo;', "'");
        return str_replace(strip_tags(strip_tags($this->contents)), '&rsquo;', "'");
    ...
    public function longPreview($max = 160){
        $max_length = strlen($this->contents);
        if($max_length > 250)
            return str_replace(strip_tags(substr($this->contents, 0,250).'...'), '&rsquo;', "'");
        return str_replace(strip_tags(strip_tags($this->contents)), '&rsquo;', "'");
