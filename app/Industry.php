...
class Industry extends Model
{
    ...
    public function getAssessmentQuestions($exp)
    {
        ...
        if($exp <= 6){
            ...
            return Question::getByDifficulty($lvl)->aptitude()->get()->random(10);
            
        }elseif($exp >6 && $exp <=36){
            // Get all the hard questions since there is no higher level
            if($lvl = 'hard'){
                return $base_questions = Question::getByDifficulty($lvl)->aptitude()->get()->random(10);
            }else{
                // Get 6 questions in the industry level
                $base_questions = Question::getByDifficulty($lvl)->aptitude()->get()->random(6);
                
                // Add 4 questions that are more difficult by one level
                return $base_questions->push(Question::getByDifficulty($newLvl)->aptitude()->get()->random(4))->flatten();
            }
        }else{
            if($lvl = 'hard'){
                return $base_questions = Question::getByDifficulty($lvl)->aptitude()->get()->random(10);
            ...
            $base_questions = Question::getByDifficulty($lvl)->aptitude()->get()->random(4);

            // Add 6 questions that are more difficult by one level
            return $base_questions->push(Question::getByDifficulty($newLvl)->aptitude()->get()->random(6))->flatten();
            }
        }
