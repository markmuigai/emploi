											...
											<ul>
												...
												@if(Auth::user()->employer->isOnStawiPackage())
													<a href="/v2/employers/applications/{{ $post->slug }}/shortlisted">
														<li class="filter pl-2" data-filter="false">2.Shortlisted ({{ count($post->shortlisted) }}
															)
														</li>
													</a>
												@else
													<a href="/checkout?product=stawi">
														<li class="filter pl-2" data-toggle="tooltip" title="Purchase stawi to unlock this feature">
